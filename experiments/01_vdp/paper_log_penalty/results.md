...
- **batch**: 74/224 runs terminated before `T_out = 10`; median 10 iterations, max 10.
- **sequential**: 194/224 runs terminated before `T_out = 150`; median 27.5 iterations, max 150.
...
Run counts are asymmetric and favour the comparator: it draws on 640 H¹ runs against this study's 112 per mode, so it has more chances to find a good one.
