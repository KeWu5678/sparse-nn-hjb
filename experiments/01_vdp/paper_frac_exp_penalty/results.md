...
- **batch**: 37/40 cells terminated before `T_out = 10`; median 4 iterations, max 10.
- **sequential**: 40/40 cells terminated before `T_out = 150`; median 15 iterations, max 108.
...
Cell counts are asymmetric and favour the comparator: it draws on 30 H¹ cells against this study's 20 per mode, so it has more chances to find a good one.
