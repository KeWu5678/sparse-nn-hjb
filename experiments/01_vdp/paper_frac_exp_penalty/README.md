...
This study applies Algorithm 2 of `paper/paper_0805.tex` to the smooth Van der
Pol value function. The inner parameters lie on the unit sphere and the outer
penalty is `alpha * sum |c_n|^q`, with `q = 2/(k+1)`.
...
`alpha`. Every fractional-power record carries the solver identifier and `rho`;
the L1 records identify the soft-threshold solver.

Use `make paper-algorithm2-refresh` to stage, validate, archive, and replace the
complete current Algorithm 2 record set. `make paper-artifacts` regenerates the
figures and reports consumed by the manuscript.
