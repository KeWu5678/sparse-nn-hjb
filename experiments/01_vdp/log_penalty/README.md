...
Sweeps activation functions on the **smooth** Van der Pol value function
(`VDP_beta_0.1_grid_30x30`, a filled 30×30 grid over [-3,3]²) to identify which
activations best balance H1 accuracy against neuron-count sparsity under the PDAP
framework. Smooth target, smooth value function — the counterpart of the
switching-set study in `../../02_pendulum/log_penalty`. Findings are in `results.md` (generated by
`analysis.py`); the VDP optimal-control problem is in `src/OpenLoop/vdp`
(Azmi–Kalise–Kunisch, control-affine with cost `β u²`).
...
Fixed: `model.power=1.0`, `data.normalize=true`, `env.seed=42`. VDP is smooth, so
the eval is the **global** default (relative L2/H1) — no region split.

Score metric: `rel H1 × neurons` (lower is better); primary ranking uses H1-loss,
profile-insertion rows. The three representatives carried through the Key Finding
in `results.md` are `softplus` (sparse ridge), `tanh` (saturating baseline), and
`gaussian` (localized RBF).
