# Historical neuron/H1 frontier (pendulum)
...
The archived figure plotted stored relative H1 error vs. network size (number of neurons), pendulum data, **signed** model, H1 loss, **profile** insertion (the relu^k series used finite_step; pendulum activationsearch was profile-only), fixed **alpha = 1e-5** for all series. The two log-penalty series used **gamma = 10**. Each curve was the per-run growth trajectory reduced to its lower envelope (cumulative-min H1 per neuron count). The historical series were:
