...
- **frac_exp_penalty rows** (`make sweep EXPERIMENT=pendulum/frac_exp_penalty`):
  signed, finite_step insertion, `power ∈ {2, 3}` (penalty exponent
  q = 2/(p+1)), gamma=0 by design, `α ∈ {1e-3…1e-6}` selected per power by
  rest L1. Powers 2.01, 4, and 5 belong to the retired stationary-root solver
  and are not valid current evidence (ADR 0009).
- **Output**: `results.md` (tables + figures), via the local `analysis.py`.
...
The former findings mixed results from the retired coefficient solver and were
removed. A current report requires fresh normalized-measure and powers-{2,3}
runs; `results.md` records that status until the local analysis is rerun.
