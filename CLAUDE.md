...
- `experiments/` — current-paper entry points and historical study reports,
  numbered by narrative: `00_openloop` → `01_vdp` → `02_pendulum`.
  For regeneration or source recovery, read `experiments/README.md`; it
  distinguishes active generators from archived analyses and figures.
...
as `_apply_publication_style`), and do not re-declare them per script. Shared
rendering and export live in `src/plots.py` (`save_figure`); saved-run loading and
physical-coordinate evaluation live in `src/results.py`. When changing plotting,
read `docs/plotting.md` for the API, local paper boundary, and validation contract.
