#!/usr/bin/env python3
"""Build physical-coordinate region sidecars for current paper records.
...
The command fails if any record cannot be rescored; partial sidecar sets must
never feed the manuscript generators.
"""
...
TUBE_RADIUS = 0.3
SCORING_VERSION = "fixed_tube_common_pool_v1"
...
def rescore_run(record_path: Path, pool_cache: dict, norm_cache: dict,
    ...
    if out_path.exists() and not force:
        ...
        if (
            cached.get("scoring_version") == SCORING_VERSION
            and cached.get("record_sha256") == _sha256(record_path)
            and cached.get("fit_history_sha256") == _sha256(pkl)
        ):
            return "cached"
    ...
    metrics = {
        ...
        "fit_history_sha256": _sha256(pkl),
    ...
    for sweep_dir in args.sweep_dirs:
        for record_path in sorted(sweep_dir.glob("**/*.json")):
            ...
            try:
                ...
                status = "failed"
                failures.append((record_path, exc))
            counts[status] = counts.get(status, 0) + 1
    print(counts)
    if failures:
        print(f"region rescoring failed for {len(failures)} record(s)")
        return 1
    return 0
