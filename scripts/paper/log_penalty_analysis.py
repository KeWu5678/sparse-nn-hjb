#!/usr/bin/env python3
"""Shared Algorithm 1 log-penalty study analysis.
...
Each study directory keeps a thin ``analysis.py`` entry point that supplies its
problem name and output directory here.
"""
...
def generate(problem: str, output_dir: Path) -> None:
    """Generate one problem's Algorithm 1 report and frontier figure."""
    if problem not in {"vdp", "pendulum"}:
        raise ValueError(f"problem must be 'vdp' or 'pendulum', got {problem!r}")
    global OUTPUT_DIR, PROBLEM, SWEEP_ROOT, FIGURES
