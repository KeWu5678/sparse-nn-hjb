...
attainable error. On the Van der Pol problem the $\ell^1$ baseline is the less
...
switching set, only the homogeneous $\mathrm{ReLU}^k$ family yields a feedback
law that follows the reference.
...
The nonhomogeneous activations are fitted by Algorithm~\ref{alg:log} at the
common operating point $(\alpha,\gamma,p)=(10^{-4},10,2.01)$; $\mathrm{ReLU}^2$
and the $\ell^1$ baseline are fitted by Algorithm~\ref{alg:q}, the latter at
...
\end{equation}
and inserted with the coefficient $c(\omega)$ of
\eqref{eq:normalized-inserted-mass}, which by
\eqref{eq:quantitative-insertion-decrease} decreases $J^M$ by at least
$ \frac{\Delta(\mu_t,\omega)^2}{(2(B^M_p)^2)}$. This holds at every accepted candidate,
not only at a global maximizer of $|P^M_{p,\mu_t}|$, so a local search
suffices for the one-step decrease. The increment bound
\eqref{eq:normalized-objective-increment} also holds at an existing atom by
subadditivity, so an exact repetition may be merged with the existing
coefficient instead of being added as a second atom.
...
\end{equation*}
over $0\le\lambda\le R(\mu_t)$ along each retained direction. Under the
standing growth bound, Theorem~\ref{thm:quantitative-insertion} gives the
radius $R(\mu_t)$ and attainment of the global maximum of
$|P^M_{p,\mu_t}|$.
...
multiple initial directions and local L-BFGS refinement, followed by the removal of near-duplicates: writing
$\widehat\omega$ for the unit representative of a refined candidate, two
candidates are \emph{near-duplicates} when
$\widehat\omega_i\cdot\widehat\omega_j>1-\theta_{\mathrm{merge}}$ for a fixed
tolerance $\theta_{\mathrm{merge}}>0$, and of each such group only the first
in the enumeration is retained. For these candidates, the preceding local
decrease estimate applies. The rate in Theorem~\ref{thm:quantitative-insertion} additionally
...
$\boldsymbol\omega$ and solves the resulting finite-dimensional coefficient
problem by the semismooth Newton method applied to the proximal residual
\begin{equation}
\label{eq:proximal-residual}
	c=\prox_{\sigma\alpha\phi}
	\bigl(c-\sigma\nabla_cl^M(\mu_{\boldsymbol\omega,c})\bigr),
	\qquad
	\prox_{\sigma\alpha\phi}(v)
	:=\argmin_{z\in\mathbb R}
	\Bigl\{\tfrac12|z-v|^2+\sigma\alpha\phi(|z|)\Bigr\},
\end{equation}
with a fixed $\sigma>0$, the scalar proximal acting coordinatewise, and
$\phi$ standing for the penalty in force, $\phi_\gamma$ or $\psi_k$. For
$\phi_\gamma$ this is the construction of \cite{pieper2022nonconvex}, whose
penalty has finite slope at the origin. For $\psi_k$ with $q<1$ the map
$c\mapsto|c|^q$ is not locally Lipschitz at the origin and that construction
does not apply: the scalar proximal is then a one-dimensional problem of the
form treated in \eqref{eq:insertion-weight-equation} below, vanishing beneath
a threshold and equal to the larger stationary point above it, and the
correction is used as a local method for which no convergence guarantee is
claimed.
...
$N_{\mathrm{ins}}\ge1$ of them together. The decrease estimates above are
...
\end{equation*}
which no individual test controls. For $N_{\mathrm{ins}}=1$ the guarantees of
Theorem~\ref{thm:quantitative-insertion} and of the finite-step criterion
apply as stated; for $N_{\mathrm{ins}}>1$ they do not, and neither a decrease
per outer iteration nor monotonicity of $J^M$ along the iterates is claimed.
Restoring the guarantee requires recomputing the residual after each
insertion, or testing the joint increment of the whole batch.
...
$\widehat\omega\in\mathbb{R}^{d+1}$.
\State Refine the directions by L-BFGS applied to
...
$\Delta(\mu_t,\omega)$, merge any exact repetition, and insert each with
coefficient $c(\omega)$.
\State Apply the semismooth Newton correction \eqref{eq:proximal-residual} to
...
given a candidate $\omega$, we evaluate the objective change
\begin{equation*}
...
\end{equation*}
and we accept $\omega$ if $\min_c\Delta J^M_{\psi_k}(c;\omega)<0$.

The minimizing coefficient has sign $-\sign(P^M_{\mu_t}(\omega))$, and only
that sign can decrease the objective; writing $A:=\|K(\omega)\|_M^2>0$, its
magnitude solves
\begin{equation}
...
root, $\Delta J^M_{\psi_k}(\cdot;\omega)$ is nondecreasing and $\omega$ is
rejected; otherwise there are two roots, and the larger one $z^*$ is the only
positive local minimizer of the increment. We compute $z^*$ by Newton's method
to the right of $z_{\mathrm{turn}}$, where the left-hand side is increasing
and convex, so the iterates decrease monotonically to it, and we accept
$\omega$ with initial outer weight
$c^*:=-\sign(P^M_{\mu_t}(\omega))z^*$ when
$\Delta J^M_{\psi_k}(c^*;\omega)<0$.
...
\State Refine them on $\mathbb S^d$ by constrained L-BFGS applied to
$|P^M_{\mu_t}(\omega)|$, and remove near-duplicate candidates.
\State For each candidate compute $c^*$ from
\eqref{eq:insertion-weight-equation} when it exists, and keep the candidate
when $\Delta J^M_{\psi_k}(c^*;\omega)<0$; terminate if there is none.
\State Retain the at most $N_{\mathrm{ins}}$ kept candidates with the smallest
$\Delta J^M_{\psi_k}(c^*;\omega)$ and insert each with outer weight $c^*$.
\State Minimize \eqref{eq:homogeneous-empirical-objective} in the coefficients
by the semismooth Newton method \eqref{eq:proximal-residual}; accept the
...
All experiments use $T_{\mathrm{out}} = 150$ outer iterations,
$N_{\mathrm{trial}} = 50$ sampled candidate nodes per iteration. Each iteration therefore inserts the single maximizing candidate ($N_{\mathrm{ins}} = 1$), which is the case
in which the guarantees of Theorem~\ref{thm:quantitative-insertion} apply, and
not the batch heuristic discussed in Section~5. The loop terminates when no
candidate clears the insertion threshold, which on these problems occurs well
before $T_{\mathrm{out}}$.
...
\paragraph{\textbf{Support confinement.}}
For a finite atomic $\mu$ write
\begin{equation*}
	R_{\max}(\mu):=\max\{|\omega| : \omega\in\atom\mu\}
\end{equation*}
for the radius of its support. Lemma~\ref{lem:support-bound} confines that
support to a compact superlevel set of $|\bar P_p|$, so $R_{\max}$ is finite;
...
\begin{equation*}
	...
	p\in\{2.01,2.5,3,4\},
\end{equation*}
on both benchmarks. Across the resulting sixteen
(problem, activation, $\alpha$) families, $R_{\max}$ decreases from $p=2.01$
to $p=4$ in every family, and decreases monotonically in $p$ in twelve of
them. The reduction is large: the widest supports contract from
$R_{\max}=17.48$ and $13.96$ (\,$\mathrm{GELU}^2$ at $p=2.01$ on the pendulum
and on Van der Pol\,) to $1.54$ and $1.67$ at $p=4$, and no family exceeds
$R_{\max}=2.10$ at $p=4$. Figure~\ref{fig:p_study} shows the sixteen curves.

Two nearby quantities do \emph{not} follow this pattern, and we record that
explicitly. The amplitude-mass radius $R_{0.95}$, defined for a discrete
measure $\mu=\sum_jc_j\delta_{\omega_j}$ by
\begin{equation*}
	R_{0.95}(\mu):=\inf\left\{R\ge0:
	|\mu|(B_R)\ge0.95\,|\mu|(\Omega)\right\},
\end{equation*}
is monotone in only four of the sixteen families, and the support size in only
five. Both are statistics of the coefficient distribution rather than of the
support, and the theory bounds the support. Increasing $p$ confines the
support; it does not reliably shrink it.
...
Colour identifies the activation and line style the value of $\alpha$. Every
family contracts as $p$ grows. All curves use seed $42$.}
\label{fig:p_study}
\end{figure}
...
Restricting the search to the certified region changes nothing. Repeating the
sixteen families with the search bounded by
$\min\{R(\mu_t),e^5\}$ instead of $e^5$ leaves the support size and the
validation error identical in all thirty-two paired runs, to machine
precision. The explanation is visible in the fits themselves: the largest
$R_{\max}$ over all of these runs is $13.96$, an order of magnitude inside
both bounds, so neither is ever active. The confinement supplied by the
moment weight is what keeps the support bounded in practice; the theoretical
radius certifies a region the iterates never approach.
...
\begin{table}[htbp]
\centering
\caption{Effect of gradient-augmented fitting at the common operating point
...
\midrule
softplus & $1.06 \times 10^{-1}$ & $4.37 \times 10^{-1}$ \\
tanh & $1.11 \times 10^{-1}$ & $4.24 \times 10^{-1}$ \\
Gaussian & $6.63 \times 10^{-2}$ & $4.22 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
...
Activation & $L^2$ trained & $H^1$ trained \\
\midrule
softplus & $5.85 \times 10^{-1}$ & $1.31 \times 10^{-1}$ \\
tanh & $6.18 \times 10^{-1}$ & $1.07 \times 10^{-1}$ \\
Gaussian & $5.42 \times 10^{-1}$ & $1.05 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
...
\caption{softplus\\ ($N=15$, $\mathrm{Err}_{H^1} = 0.131$).}
...
\caption{$\tanh$\\ ($N=23$, $\mathrm{Err}_{H^1} = 0.107$).}
...
\caption{Gaussian\\ ($N=32$, $\mathrm{Err}_{H^1} = 0.105$).}
...
\midrule
softplus & 6 & $1.06 \times 10^{-1}$ \\
tanh & 8 & $1.11 \times 10^{-1}$ \\
Gaussian & 10 & $6.63 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\caption{$L^2$ trained}
...
\midrule
softplus & 15 & $1.31 \times 10^{-1}$ \\
tanh & 23 & $1.07 \times 10^{-1}$ \\
Gaussian & 32 & $1.05 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
\caption{$H^1$ trained}
...
$5.99\times10^{-1}$ to $9.09\times10^{-2}$ for softplus, from
$6.33\times10^{-1}$ to $5.34\times10^{-2}$ for $\tanh$, and from
$5.55\times10^{-1}$ to $5.04\times10^{-2}$ for the Gaussian. Fitting values
alone leaves the gradient essentially unapproximated, and the resulting
feedback law would be unusable.

The Gaussian is most accurate ($1.05\times10^{-1}$) and uses $32$ atoms;
$\tanh$ reaches $1.07\times10^{-1}$ with $23$, and softplus
$1.31\times10^{-1}$ with only $15$. The three are separated by less in
...
The Gaussian reaches $9.71\times10^{-2}$ with $54$ atoms, while
$\mathrm{ReLU}^2$ and $\mathrm{ReLU}^5$ reach comparable errors with roughly
$40$ and $21$ atoms. Softplus stops at $1.23\times10^{-1}$ with $66$ atoms,
so the homogeneous profiles reach the Gaussian's accuracy on a smaller
support.
\begin{figure}[htbp]
\centering
...
optimality.
Figure~\ref{fig:summary_feedback} shows $\|y(t)\|$ and $|u(t)|$ for softplus, the Gaussian, and $\reluk$ ($k = 5$), beside this reference. All three drive the state to the origin and track the reference control, at essentially its closed-loop cost ($6.54$, $6.50$, $6.50$ vs.\ $6.48$).
\begin{figure}[htbp]
\centering
\captionsetup[subfigure]{justification=centering}
\begin{subfigure}[t]{0.48\textwidth}
...
$b=0$ shown in light green. The two nonhomogeneous fits use the common
operating point $\alpha=10^{-4}$, $\gamma=10$, $p=2.01$; dot color gives the
...
may differ; the sampled PMP costates provide branchwise labels, without
asserting a classical gradient on $S$.
...
No run places atoms at the upper radial bound $\lambda=e^5$: the largest
support radius over all reported nonhomogeneous fits is $13.96$, and at the
common operating point none exceeds $4.5$. The weight $w_p$ confines the
support on its own, so no run has to be excluded on this ground.
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/surface_relu2.png}
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/surface_relu3.png}
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/surface_relu5.png}
...
\midrule
Gaussian & 6k, $23\%$ near $S$ & $6.23 \times 10^{-1}$ & $6.34 \times 10^{-1}$ & 140 \\
 & 6k, $40\%$ near $S$ & $6.27 \times 10^{-1}$ & $6.18 \times 10^{-1}$ & 142 \\
 & 6k, $60\%$ near $S$ & $6.61 \times 10^{-1}$ & $6.58 \times 10^{-1}$ & 42 \\
 & 6k $+$ 2k near $S$ & $6.39 \times 10^{-1}$ & $6.33 \times 10^{-1}$ & 138 \\
\midrule
$\mathrm{ReLU}^{2}$ & 6k, $23\%$ near $S$ & $2.70 \times 10^{-1}$ & $1.63 \times 10^{-1}$ & 110 \\
 & 6k, $40\%$ near $S$ & $3.13 \times 10^{-1}$ & $1.89 \times 10^{-1}$ & 133 \\
 & 6k, $60\%$ near $S$ & $3.48 \times 10^{-1}$ & $2.10 \times 10^{-1}$ & 112 \\
 & 6k $+$ 2k near $S$ & $2.78 \times 10^{-1}$ & $1.75 \times 10^{-1}$ & 133 \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:rs_feedback_cost};
the rectified powers use $\psi_k$, with $\alpha=10^{-5}$ for
$k=2$ and $\alpha=10^{-6}$ for $k\in\{3,5\}$. The error for
$\mathrm{ReLU}^{2}$ becomes
smaller than the other errors within the first insertions and reaches
$\approx2.6\times10^{-1}$. $\mathrm{ReLU}^{3}$ is second, while softplus,
...
$\mathrm{ReLU}^{3}$. Among the nonhomogeneous profiles the Gaussian has the
smallest errors, followed by $\tanh$ and then softplus.
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/feedback_relu2.png}
...
\begin{subfigure}[b]{0.32\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/feedback_relu5.png}
...
$\tanh$ & $264.7$ & no & $258.4$ & no \\
\midrule
$\mathrm{ReLU}^{2} + \psi_{2}$ & $55.5$ & yes & $10.1$ & yes \\
$\mathrm{ReLU}^{3} + \psi_{3}$ & diverged & no & $10.2$ & yes \\
$\mathrm{ReLU}^{5} + \psi_{5}$ & $218.8$ & no & $216.1$ & no \\
\bottomrule
\end{tabular}
...
Algorithm~\ref{alg:log} at the common operating point
$\alpha = 10^{-4}$, $\gamma = 10$, $p = 2.01$; the lower block uses
Algorithm~\ref{alg:q} at the most accurate $\alpha$ for each $k$. For
$\mathrm{ReLU}^{3}$ from $A$ the rollout diverges, so no cost is reported.
The claim that only $\mathrm{ReLU}^{2}$ synthesizes a law reaching upright
from both starts is scoped to this fixed operating point; selecting each
activation's own best cell admits further laws that reach upright from $B$.}
...
\begin{subfigure}[b]{0.48\textwidth}
\centering
\includegraphics[width=\textwidth]{../experiments/02_pendulum/paper_frac_exp_penalty/figures/feedback_control_b_relu.png}
...
reference: it is the only law that reaches the upright from both starts, at
cost $10.1$ from $B$ and $55.5$ from $A$ against the reference $10.2$ and
$26.2$. $\mathrm{ReLU}^{3}$ succeeds only from $B$, and
$\mathrm{ReLU}^{5}$ from neither.

None of the nonhomogeneous activations synthesizes a working law here, from
either start. The reason is visible in the error split rather than in the
support size: over the whole sweep their gradient error is insensitive to
width, holding near $7.0\times10^{-1}$ for softplus, $6.2\times10^{-1}$ for
$\tanh$ and $6.0\times10^{-1}$ for the Gaussian as the support grows from
fifteen atoms to over a hundred, while their value error does improve. The
$H^1$ error is dominated by the gradient term, and the feedback law is built
from $\nabla\widehat V$, so no amount of width repairs it. On the same data
$\mathrm{ReLU}^{2}$ attains gradient error $2.6\times10^{-1}$. A smooth
activation cannot represent a gradient that jumps across $S$; the
$k$-homogeneous family can, which is the same conclusion the value surfaces of
Section~\ref{sec:relu2-switching} reach by a different route. Adding samples
near $S$ does not reverse it.
...
For the Van der Pol oscillator, raising the moment order $p$ contracts the
support radius in every tested family, which is the confinement the
normalization is introduced to provide; the amplitude-mass radius and the
support size do not follow it. The Gaussian gives the smallest $H^1$ error
...
complement for every training set, whereas the Gaussian errors are nearly
equal on the two regions throughout. Adding training
samples near $S$ does not reduce the error on $S_{0.3}$ at the tested network
widths.
$\mathrm{ReLU}^{2}$ gives the smallest error and the closed-loop path closest
to the reference, and is the only law reaching the upright from both initial
states. None of the nonhomogeneous profiles synthesizes a working law here.
Their gradient error is insensitive to width, so the deficiency is one of
approximation rather than of capacity: a smooth activation cannot represent a
gradient that jumps across $S$, and the feedback law is built from that
gradient.
