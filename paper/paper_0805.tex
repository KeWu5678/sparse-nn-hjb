...
attainable error. On the Van der Pol problem the traditional $\ell^1$ network is
less accurate than the best nonconvex curve at every support budget below $114$
atoms --- $1.70\times10^{-1}$ against $1.02\times10^{-1}$ at twenty atoms ---
and overtakes it only past that width, eventually reaching
$8.75\times10^{-2}$. The nonconvex penalties therefore buy accuracy per neuron,
not a lower floor. On the pendulum, where the value gradient jumps across the
switching set, the Gaussian gives the smallest regional errors, while softplus
and $\mathrm{ReLU}^{3}$ reach an upright neighbourhood from both tested starts.
...
\caption{$k=2$\\ ($N=48$, $\mathrm{Err}_{H^1} = 0.099$).}
...
Power & $N$ & $\mathrm{Err}_{L^2}$ & $\mathrm{Err}_{H^1}$ \\
\midrule
$k = 2$ & 48 & $4.17 \times 10^{-1}$ & $9.92 \times 10^{-2}$ \\
$k = 3$ & 27 & $4.15 \times 10^{-1}$ & $9.95 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\caption{$H^1$ trained}
...
$9.83\times10^{-2}$. The lowest-error Algorithm~2 checkpoints use $48$ atoms
at $9.92\times10^{-2}$ for $\mathrm{ReLU}^2$ and $38$ atoms at
$9.71\times10^{-2}$ for $\mathrm{ReLU}^3$. Thus all four reach the same
...
$\alpha=10^{-5}$ for $k=2$ and $\alpha=10^{-6}$ for $k=3$.}
...
\midrule
$\mathrm{ReLU}^{2}$ & 6k, $23\%$ near $S$ & $3.20 \times 10^{-1}$ & $2.80 \times 10^{-1}$ & 140 \\
 & 6k, $40\%$ near $S$ & $3.24 \times 10^{-1}$ & $2.26 \times 10^{-1}$ & 139 \\
 & 6k, $60\%$ near $S$ & $3.55 \times 10^{-1}$ & $2.80 \times 10^{-1}$ & 144 \\
 & 6k $+$ 2k near $S$ & $3.64 \times 10^{-1}$ & $3.18 \times 10^{-1}$ & 59 \\
\bottomrule
\end{tabular}
...
$\alpha=10^{-6}$ for $k=3$. The Gaussian reaches the smallest validation
error, approximately $3.61\times10^{-1}$, followed by
$\mathrm{ReLU}^{2}$ at $3.93\times10^{-1}$, $\tanh$ at
$4.06\times10^{-1}$, $\mathrm{ReLU}^{3}$ at $4.78\times10^{-1}$, and
...
$J^M_{\psi_k}$ with $\alpha=10^{-5}$ for $k=2$ and $\alpha=10^{-6}$ for
$k=3$.}
...
$0.359$ and $0.289$ for $\mathrm{ReLU}^{2}$, $0.358$ and $0.197$ for
$\tanh$, $0.511$ and $0.521$ for $\mathrm{ReLU}^{3}$, and $0.501$ and
$0.417$ for softplus.
...
Softplus and $\mathrm{ReLU}^{3}$ reach an upright neighbourhood from both
...
softplus and $\mathrm{ReLU}^{3}$, and they do so at different costs
(Table~\ref{tab:rs_feedback_cost}). Softplus reaches the upright at costs
$69.8$ and $11.2$ from $A$ and $B$, while $\mathrm{ReLU}^{3}$ reaches it at
costs $110.9$ and $12.6$. The Gaussian, $\tanh$, and $\mathrm{ReLU}^{2}$
satisfy the terminal criterion only from $B$; there $\mathrm{ReLU}^{2}$ has
cost $10.5$ against the reference cost $10.2$.
...
\midrule
$\mathrm{ReLU}^{2} + \psi_{2}$ & $498.2$ & no & $10.5$ & yes \\
$\mathrm{ReLU}^{3} + \psi_{3}$ & $110.9$ & yes & $12.6$ & yes \\
\bottomrule
\end{tabular}
...
$\mathrm{ReLU}^{2}$ from $A$ the rollout misses the terminal criterion and
accumulates a large cost.}
...
Softplus and $\mathrm{ReLU}^{3}$ reach the upright from both starts, at costs
$69.8$ and $110.9$ from $A$ against the reference $26.2$. The Gaussian,
$\tanh$, and $\mathrm{ReLU}^{2}$ succeed only from $B$; among the rectified
models, $\mathrm{ReLU}^{2}$ is closer to the reference there ($10.5$ vs.\
$10.2$).
...
decision from $A$ while they do not. Conversely, $\mathrm{ReLU}^{3}$ has
larger regional errors than $\mathrm{ReLU}^{2}$ but succeeds from $A$, while
$\mathrm{ReLU}^{2}$ does not. Thus the switching-set error diagnoses the
difficulty of approximating the discontinuous gradient, but a reliable
...
The Gaussian gives the smallest regional errors. Softplus and
$\mathrm{ReLU}^{3}$ reach an upright neighbourhood from both initial states,
while the Gaussian, $\tanh$, and $\mathrm{ReLU}^{2}$ do so only from $B$.
