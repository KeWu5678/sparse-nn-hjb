...
which exists because a continuous map on a compact metric space is bounded,
strongly measurable and separably valued; for a purely atomic
...
$L_\phi=\phi'(0+)$ is finite and positive. For the $k\ge2$
...
Let $k\ge2$, $q=2/(k+1)\in(0,1)$, $\alpha>0$, $V\in H^1(D)$, and let
$K:\mathbb S^d\to H^1(D)$ be continuous, with
$C_S=\sup_{\omega\in\mathbb S^d}\|K(\omega)\|_{H^1(D)}$.
\begin{enumerate}
	\item If $V=0$ or $C_S=0$, then $\bar\mu=0$ is the unique global minimizer
	of \eqref{prob:homogeneous} and its reduced width is $0$.
	\item If $V\ne0$ and $C_S>0$, then \eqref{prob:homogeneous} admits a
	global minimizer, and its minimum value equals the finite-network infimum
	on the right-hand side of \eqref{eq:homogeneous-infima}. Every global
	minimizer $\bar\mu$ is a finite atomic measure whose reduced width
	satisfies
	\begin{equation*}
		N\le N_{\max}:=
...
\end{theorem}
\begin{proof}
We first dispose of the degenerate cases, then prove existence for the
finite-network problem in three steps, and finally transfer the conclusion to
\eqref{prob:homogeneous} by Proposition~\ref{prop:homogeneous_reduction}.

\emph{Step 0: the degenerate cases.} If $V=0$, then $J_{\psi_k}(0)=0$, and
every $\mu\ne0$ with $J_{\psi_k}(\mu)<\infty$ has
$J_{\psi_k}(\mu)\ge\alpha\Phi_{\psi_k}(\mu)>0$ by
Lemma~\ref{lem:homogeneous-penalty-properties}(2). If $C_S=0$, then
$K\equiv0$ on $\mathbb S^d$, so $\mathcal N\mu=0$ and
$L(\mu)=\frac12\|V\|^2_{H^1(D)}$ for every $\mu\in\mathcal M(\mathbb S^d)$,
and again the penalty is minimized only at $\mu=0$. In both cases $\mu=0$ is
the unique global minimizer and $\#\atom 0=0$. Assume from now on that
$V\ne0$ and $C_S>0$; then $c_{\min,\mathrm{glob}}\in(0,\infty)$ and
...
repeated. Lemma~\ref{prop:homogeneous_penalty} and
