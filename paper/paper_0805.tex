...
The comparison is one of accuracy at a given support size rather than of
lowest attainable error: the $\ell^1$ baseline produces no fit below roughly
ninety atoms on either problem, while the Gaussian and $\mathrm{ReLU}^2$ reach
comparable accuracy on a fraction of that support. On the pendulum, where the
value gradient jumps, only the homogeneous $\mathrm{ReLU}^k$ family follows
...
$N_{\mathrm{trial}} = 50$ sampled candidate nodes per iteration,
$N_{\mathrm{ins}} = 1$, and $\varepsilon_{\mathrm{prune}} = 10^{-8}$. Each
iteration therefore inserts the single maximizing candidate, which is the case
