...
All experiments use $T_{\mathrm{out}} = 10$ outer iterations,
$N_{\mathrm{trial}} = 50$ sampled candidate nodes per iteration,
$N_{\mathrm{ins}} = 15$, and $\varepsilon_{\mathrm{prune}} = 10^{-8}$. The
insertion batches are therefore of the heuristic kind discussed in Section~5.
...
\end{equation*}
for the radius of its support, which is the quantity
Lemma~\ref{lem:support-bound} bounds. We use Algorithm~\ref{alg:log} with
