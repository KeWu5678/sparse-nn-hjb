...
The displayed nonconvex models reach lower error with fewer neurons. The
traditional $\ell^1$ network approaches their error as its support grows, but
remains above them through the plotted budget of $150$ neurons.
Figure~\ref{fig:neuron_h1_frontier}\subref{fig:feedback_cost_a}
reports the complementary closed-loop test on the pendulum problem. Only
$\mathrm{ReLU}^{3}$ reaches the upright neighbourhood from that start; the
remaining selected models do not. The start $A$ lies at distance $0.247$ from
the switching set, inside the neighbourhood $S_{0.3}$ where all five fits have
higher relative $H^1$ error than away from the switching set. The feedback law
depends on the learned gradient, so the outcome tests its accuracy near the
gradient discontinuity. Rollout costs must be read together with the terminal
criterion: a lower cost does not by itself establish stabilization.
...
$\tanh$ & $1478.1$ & no \\
\midrule
$\mathrm{ReLU}^{2} + \psi_{2}$ & $641.3$ & no \\
$\mathrm{ReLU}^{3} + \psi_{3}$ & $230.7$ & yes \\
\bottomrule
\end{tabular}
\caption{Pendulum feedback from start $A$.}
\label{fig:feedback_cost_a}
\end{subfigure}
\caption{Panel (a) shows the best observed relative $H^1$ validation error
at each support budget. Panel (b) gives
...
with $(\alpha,\gamma,p)=(10^{-4},10,2.01)$. The ReLU--$\ell^1$ baseline in
panel (a) uses classical profile-threshold insertion with a convex $\ell^1$
penalty at $\alpha=10^{-5}$; the homogeneous model in that panel uses
$\alpha=10^{-6}$. The homogeneous models in panel (b) use $\alpha=10^{-5}$,
selected by the smallest relative $H^1$ error on $S_{0.3}$ among the tested
values $\{10^{-3},10^{-4},10^{-5},10^{-6}\}$. Each feedback law uses its run's
minimum-training-objective iterate. These are benchmark-specific comparisons.}
...
and different insertion methods are chosen given the problem setting.
...
In both algorithms the coefficient-correction step freezes the locations
$\boldsymbol\omega$ and applies a semismooth Newton method to the resulting
finite-dimensional coefficient problem. They are specified in
...
$\|P_{p,\mu}\|_\infty$ when $\Delta(\mu)>0$.
...
the corresponding sampled feature-growth constants.
...
points at each iteration. Second, it gives an initial outer weight that
guarantees a single-atom insertion decrease before the local semismooth Newton
correction.
...
Newton method. If the correction increases $J^M$, discard it and keep the
post-insertion weights.
...
$q\in\{2/3,1/2\}$. The ReLU--$\ell^1$ baseline reported alongside it is not this
algorithm at $k=1$: it is the classical profile-threshold insertion with a
coordinate-descent warm start and the convex penalty $\alpha\sum_i|c_i|$, run
separately.
...
\eqref{eq:algorithm2-prox-scale}. If the correction increases
$J^M_{\psi_k}$, discard it and keep the inserted coefficients $c^*$.
\State Remove nodes with $|c_n|\le\varepsilon_{\mathrm{prune}}$ (resulting in
width $N(t+1)$).
\State $t = t + 1$.
\EndWhile
...
specified for each experiment, using the original-scale values and gradients
with respect to the original state variables after undoing data preprocessing.
The measure/dictionary normalization and penalty in the training objective do
not enter these reporting errors. Except for the best-error frontiers, we
evaluate the recorded iterate with the smallest training objective.
For a frontier, support sizes are sorted and the reported error is the
smallest among recorded iterates with no more atoms; it is not an iteration
trace.
...
Both algorithms use $T_{\mathrm{out}} = 150$ outer iterations and
...
one-candidate decrease statement applies. The ReLU--$\ell^1$ baseline keeps its
own established settings: batch insertion with $N_{\mathrm{ins}} = 15$ over
$T_{\mathrm{out}} = 10$ iterations and an unguarded correction, which admits the
same maximum width of $150$ atoms. Corollary~\ref{cor:certificate-rate}
...
(problem, activation, $\alpha$) families. In fourteen families $R_{\max}$
decreases monotonically over the tested orders. The largest absolute
contraction is for softplus on the pendulum at $\alpha=10^{-5}$, from
$147.34$ at $p=2.01$ to $5.08$ at $p=4$; across all families the largest
radius at $p=4$ is $12.88$.
...
coefficient distribution. They decrease monotonically in ten and twelve of
the sixteen families, respectively, so their behavior is reported separately
rather than inferred from the support theorem.
...
Colour identifies the activation and line style the value of $\alpha$.
Fourteen of the sixteen support-radius curves decrease monotonically; all
use seed $42$.}
\label{fig:p_study}
\end{figure}
...
$1.1\times10^{-3}$, and their sign is mixed. Because the radius determines
...
\midrule
softplus & $1.06 \times 10^{-1}$ & $4.26 \times 10^{-1}$ \\
tanh & $7.86 \times 10^{-2}$ & $4.17 \times 10^{-1}$ \\
Gaussian & $7.39 \times 10^{-2}$ & $4.17 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
...
Activation & $L^2$ trained & $H^1$ trained \\
\midrule
softplus & $5.00 \times 10^{-1}$ & $2.43 \times 10^{-1}$ \\
tanh & $4.77 \times 10^{-1}$ & $2.37 \times 10^{-1}$ \\
Gaussian & $4.76 \times 10^{-1}$ & $2.36 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
...
\caption{softplus\\ ($N=18$, $\mathrm{Err}_{H^1} = 0.243$).}
...
\caption{$\tanh$\\ ($N=33$, $\mathrm{Err}_{H^1} = 0.237$).}
...
\caption{Gaussian\\ ($N=39$, $\mathrm{Err}_{H^1} = 0.236$).}
...
accuracy. Predictions are clipped to $[0,20]$ for visualization;
the reported errors use the unclipped network outputs.}
...
tanh & 10 & $7.86 \times 10^{-2}$ \\
Gaussian & 9 & $7.39 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\caption{$L^2$ trained}
...
\midrule
softplus & 18 & $2.43 \times 10^{-1}$ \\
tanh & 33 & $2.37 \times 10^{-1}$ \\
Gaussian & 39 & $2.36 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
\caption{$H^1$ trained}
...
alone therefore gives substantially poorer gradient approximations, which
can compromise feedback synthesis. These gradient gains come at the expense
of value accuracy (Table~\ref{tab:alg1_gradient}).

The Gaussian is most accurate ($2.36\times10^{-1}$) and uses $34$ atoms;
$\tanh$ reaches $2.37\times10^{-1}$ with $38$, and softplus
$2.41\times10^{-1}$ with only $16$. The three are separated by less in
...
\caption{$k=2$\\ ($N=53$, $\mathrm{Err}_{H^1} = 0.235$).}
...
\caption{$k=3$\\ ($N=26$, $\mathrm{Err}_{H^1} = 0.235$).}
\label{fig:H1_power_p3}
\end{subfigure}
\caption{$H^1$-trained networks with $\alpha = 10^{-5}$ for
$k \in \{2,3\}$. Predictions are clipped to $[0,20]$ for visualization;
the reported errors use the unclipped network outputs.}
\label{fig:H1_power}
\end{figure}

For these fixed-$\alpha$, single-seed fits, adding gradient data lowers the
combined value--gradient error from
$\mathrm{Err}_{H^1}\approx 3.5\times10^{-1}$ to
$\approx 2.4\times10^{-1}$ for both powers, a factor of about $1.5$
(Table~\ref{tab:alg2_gradient}). This improvement comes with larger support
and higher value error $\mathrm{Err}_{L^2}$; the comparison alone does not
establish better closed-loop performance.
...
\midrule
$k = 2$ & 15 & $2.77 \times 10^{-2}$ & $3.59 \times 10^{-1}$ \\
$k = 3$ & 13 & $3.37 \times 10^{-2}$ & $3.91 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
\caption{$L^2$ trained}
...
Power & $N$ & $\mathrm{Err}_{L^2}$ & $\mathrm{Err}_{H^1}$ \\
\midrule
$k = 2$ & 53 & $4.16 \times 10^{-1}$ & $2.35 \times 10^{-1}$ \\
$k = 3$ & 26 & $4.15 \times 10^{-1}$ & $2.35 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
\caption{$H^1$ trained}
...
$\mathrm{Err}_{H^1} \approx 3.5 \times 10^{-1}$
(Table~\ref{tab:alg2_gradient}\,(a)).

\subsubsection{Summary}

Figure~\ref{fig:summary_frontier} plots the best observed relative $H^1$ error
at each support budget along the selected fits. At the shared Algorithm~1 operating point, softplus
uses $16$ atoms at error $2.41\times10^{-1}$ and the Gaussian uses $34$ at
$2.36\times10^{-1}$. The selected minimum-training-objective Algorithm~2
checkpoints use $91$ atoms at $2.35\times10^{-1}$ for $\mathrm{ReLU}^2$
and $37$ atoms at $2.35\times10^{-1}$ for $\mathrm{ReLU}^3$.
Thus all four reach the same
...
\caption{Best observed relative $H^1$ validation error against support budget.
...
optimality.
Figure~\ref{fig:summary_feedback} shows $\|y(t)\|$ and $|u(t)|$ for softplus, the Gaussian, and $\reluk$ ($k = 3$), beside this reference. All three drive the state to the origin, with closed-loop costs $6.68$, $6.49$, and $6.50$, respectively, compared with the reference cost $6.48$.
\begin{figure}[htbp]
\centering
\captionsetup[subfigure]{justification=centering}
\begin{subfigure}[t]{0.48\textwidth}
...
window $[-8,8]^{2}$. Predictions below $0$ or above $60$ are clipped to
the respective boundary for visualization; errors and feedback use the
unclipped fitted function.
...
\midrule
Gaussian & 6k, $23\%$ near $S$ & $2.38 \times 10^{-1}$ & $1.37 \times 10^{-1}$ \\
 & 6k, $40\%$ near $S$ & $2.75 \times 10^{-1}$ & $1.50 \times 10^{-1}$ \\
 & 6k, $60\%$ near $S$ & $3.08 \times 10^{-1}$ & $2.54 \times 10^{-1}$ \\
 & 6k $+$ 2k near $S$ & $2.85 \times 10^{-1}$ & $1.85 \times 10^{-1}$ \\
\midrule
$\mathrm{ReLU}^{2}$ & 6k, $23\%$ near $S$ & $2.62 \times 10^{-1}$ & $1.68 \times 10^{-1}$ \\
 & 6k, $40\%$ near $S$ & $2.98 \times 10^{-1}$ & $1.72 \times 10^{-1}$ \\
 & 6k, $60\%$ near $S$ & $3.08 \times 10^{-1}$ & $2.36 \times 10^{-1}$ \\
 & 6k $+$ 2k near $S$ & $2.50 \times 10^{-1}$ & $1.64 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
...
row taken from the same run. The Gaussian uses $(\gamma,p)=(1,2.01)$ and
$\alpha\in\{10^{-3},10^{-4},10^{-5}\}$; $\mathrm{ReLU}^2$ uses $\psi_2$ and
$\alpha\in\{10^{-4},10^{-5},10^{-6}\}$. Errors are computed after
un-normalizing, so the two blocks share one definition. All four training sets
are scored on one common evaluation pool, formed by removing the union of their
sampled points from the certified two-sided point set, so the rows are mutually
comparable and each is out-of-sample.}
...
tracks the best observed relative $H^{1}$ validation error at each support
budget, for each activation paired with its penalty. The three
nonhomogeneous activations use the parameter values in
Figure~\ref{fig:neuron_h1_frontier}\subref{fig:feedback_cost_a};
the rectified powers use $\psi_k$ with $\alpha=10^{-5}$ for both $k=2$ and
$k=3$. The $k=2$ fit reaches the smallest validation error, approximately
$3.00\times10^{-1}$, followed by the Gaussian at $3.23\times10^{-1}$,
$\mathrm{ReLU}^{3}$ at $3.32\times10^{-1}$, $\tanh$ at
$3.74\times10^{-1}$, and softplus at $4.74\times10^{-1}$.
...
$J^M_{\psi_k}$ with $\alpha=10^{-5}$ for both $k=2$ and $k=3$.}
...
$9.4\times10^{5}$ points. The union of the five pendulum datasets' sampled
rows is excluded, and for each run we use
...
smallest errors in both regions ($0.310$ and $0.180$). The corresponding
values are $0.335$ and $0.195$ for the Gaussian, $0.336$ and $0.216$ for
$\mathrm{ReLU}^{3}$, $0.387$ and $0.206$ for $\tanh$, and $0.479$ and
$0.389$ for softplus.
...
Only $\mathrm{ReLU}^{3}$ among the fitted laws reaches an upright
neighbourhood from both starts; the Gaussian, softplus, $\tanh$, and
$\mathrm{ReLU}^{2}$ reach one only from $B$.}
\label{fig:rs_feedback}
\end{figure}

From the harder start $A$, only $\mathrm{ReLU}^{3}$ among the fitted laws
reaches the upright neighbourhood, at cost $230.7$ against the reference
cost $26.2$ (Figure~\ref{fig:neuron_h1_frontier}\subref{fig:feedback_cost_a}).
The Gaussian, softplus, $\tanh$, and $\mathrm{ReLU}^{2}$ laws do not satisfy
the terminal criterion from this start. All five fitted laws reach the upright
neighbourhood from $B$, at costs $10.2$, $13.7$, $10.3$, $10.2$, and $10.3$,
respectively, against the reference cost $10.2$.
...
The $\mathrm{ReLU}^{2}$ fit has the smallest validation and regional errors,
yet fails the terminal criterion from $A$. The $\mathrm{ReLU}^{3}$ fit has
higher regional errors but reaches an upright neighbourhood from both starts.
The regional approximation error and the binary terminal outcome therefore
rank the models differently. Error near the switching set diagnoses the
difficulty of approximating the discontinuous gradient, while feedback
assessment still requires a closed-loop test.
