...
The nonconvex models have lower error at small support budgets, whereas the
traditional $\ell^1$ network reaches a lower error with a larger support.
Thus the observed advantage is a small-width accuracy trade-off, not a
uniform improvement across support budgets.
...
remaining selected models, including the ReLU network with $\ell^1$ penalty, do
not. The start $A$ lies at distance $0.247$ from the switching set, inside the
tube $S_{0.3}$ on which every model has its largest gradient error, and the
feedback law depends on the learned gradient alone. Outcomes from a single
start inside the tube are therefore marginal, and the cost reported for a model
that fails to stabilize measures how far the trajectory wanders rather than a
quality of the fit.
...
with $(\alpha,\gamma,p)=(10^{-4},10,2.01)$; the ReLU--$\ell^1$ baseline is the
classical profile-threshold insertion with a convex $\ell^1$ penalty, and the
homogeneous models use $\alpha=10^{-6}$.
The baseline run in (a) attains the lowest frontier error in its six-$\alpha$
sweep; the baseline checkpoint in (b) has the lowest validation error in its
six-$\alpha$ sweep at the minimum-training-objective iterate, not the lowest
rollout cost. These are benchmark-specific comparisons, not a matched-$\alpha$
comparison.}
...
$2.71\times10^{-1}$, followed by the Gaussian at $3.13\times10^{-1}$,
$\tanh$ at $3.48\times10^{-1}$, $\mathrm{ReLU}^{3}$ at
$4.05\times10^{-1}$, and softplus at $4.93\times10^{-1}$.
