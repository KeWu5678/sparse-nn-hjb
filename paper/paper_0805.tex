...
\State Sample $N_{\mathrm{trial}}$ random directions
...
\State For each retained direction, optimize
$e^{-3}\le\lambda\le\min\{R(\mu_t),e^5\}$ in
$|P^M_{p,\mu_t}(\lambda\widehat{\omega})|$; when the theorem radius is
unavailable, use $e^5$ as the upper bound.
...
which on these problems occurs well before $T_{\mathrm{out}}$.
...
explicitly. The radius $R_{0.95}$, defined for a discrete
...
support, and the theory bounds the support. Increasing $p$ contracts the
observed support radius; it does not reliably reduce these other quantities.
...
both bounds, so neither is ever active. The normalized weight keeps the
observed support bounded in these runs; the theoretical radius certifies a
region the iterates never approach.
...
support radius over all reported nonhomogeneous fits is $13.96$, and with
$\alpha=10^{-4}$, $\gamma=10$, $p=2.01$ none exceeds $4.5$. The normalized
weight keeps the support away from the numerical bound, so no run has to be
...
from both starts is scoped to these fixed parameters; selecting the
lowest-error run for each activation admits further laws that reach upright
from $B$.}
...
support radius in every tested family, as predicted by the normalized
formulation; $R_{0.95}$ and the support size do not follow it. The Gaussian gives the smallest $H^1$ error
