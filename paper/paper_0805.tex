...
switching set, the Gaussian gives the smallest regional errors, while
$\mathrm{ReLU}^{2}$ gives the feedback law closest to the reference.
Softplus also reaches an upright from both tested starts, but at a higher cost
from the harder start.
...
\subsubsection*{The two insertion algorithms}
...
unit sphere. For $q<1$, the zero network is a local minimizer and the
infinitesimal insertion condition of Algorithm~\ref{alg:log} is unavailable
(Remark~\ref{rem:homogeneous_singular_slope}). We therefore test the actual
change produced by inserting one atom with a finite coefficient.
...
undamped Newton iteration on a fixed smooth branch has the usual local rate
under nonsingularity; no convergence rate is claimed for the implemented
globalizations. The corrected coefficients are retained only when they do not
increase the post-insertion objective.
...
\State Discard final candidates that coincide numerically with the current
support.
...
\eqref{eq:algorithm2-prox-scale}; accept the
...
\caption{$k=2$\\ ($N=45$, $\mathrm{Err}_{H^1} = 0.100$).}
...
\caption{$k=3$\\ ($N=27$, $\mathrm{Err}_{H^1} = 0.099$).}
...
$k = 3$ & 12 & $3.67 \times 10^{-2}$ & $4.53 \times 10^{-1}$ \\
\bottomrule
\end{tabular}
\caption{$L^2$ trained}
...
Power & $N$ & $\mathrm{Err}_{L^2}$ & $\mathrm{Err}_{H^1}$ \\
\midrule
$k = 2$ & 45 & $4.17 \times 10^{-1}$ & $9.96 \times 10^{-2}$ \\
$k = 3$ & 27 & $4.15 \times 10^{-1}$ & $9.87 \times 10^{-2}$ \\
\bottomrule
\end{tabular}
\caption{$H^1$ trained}
...
$q=2/(k+1)$. At the same $H^1$ error scale, the $k=3$ fit uses $27$ atoms
instead of $45$ for $k=2$.
Under the $L^2$ loss the fits use $12$--$13$ atoms and have
$\mathrm{Err}_{H^1} \approx 4.1$--$4.5 \times 10^{-1}$
...
$9.83\times10^{-2}$. The lowest-error Algorithm~2 checkpoints use $75$ atoms
at $9.84\times10^{-2}$ for $\mathrm{ReLU}^2$ and $40$ atoms at
$9.69\times10^{-2}$ for $\mathrm{ReLU}^3$. Thus all four reach the same
...
$\alpha=10^{-6}$ for the displayed $k\in\{2,3\}$ checkpoints.}
...
optimality.
Figure~\ref{fig:summary_feedback} shows $\|y(t)\|$ and $|u(t)|$ for softplus, the Gaussian, and $\reluk$ ($k = 3$), beside this reference. All three drive the state to the origin and track the reference control, at essentially its closed-loop cost ($6.48$, $6.50$, $6.50$ vs.\ $6.48$).
\begin{figure}[htbp]
\centering
\captionsetup[subfigure]{justification=centering}
\begin{subfigure}[t]{0.48\textwidth}
...
\midrule
$\mathrm{ReLU}^{2}$ & 6k, $23\%$ near $S$ & $3.30 \times 10^{-1}$ & $2.94 \times 10^{-1}$ & 140 \\
 & 6k, $40\%$ near $S$ & $2.93 \times 10^{-1}$ & $2.46 \times 10^{-1}$ & 139 \\
 & 6k, $60\%$ near $S$ & $3.56 \times 10^{-1}$ & $2.94 \times 10^{-1}$ & 145 \\
 & 6k $+$ 2k near $S$ & $3.54 \times 10^{-1}$ & $2.65 \times 10^{-1}$ & 105 \\
\bottomrule
\end{tabular}
...
Table~\ref{tab:rs_feedback_cost};
the rectified powers use $\psi_k$ with $\alpha=10^{-6}$. The Gaussian reaches
the smallest validation error, approximately $3.61\times10^{-1}$, followed
by $\mathrm{ReLU}^{2}$ at $3.75\times10^{-1}$, $\tanh$ at
$4.06\times10^{-1}$, $\mathrm{ReLU}^{3}$ at $4.56\times10^{-1}$, and
softplus at $5.31\times10^{-1}$.
...
$J^M_{\psi_k}$ with $\alpha=10^{-6}$ for $k\in\{2,3\}$.}
...
on $S_{0.3}$ and $D\setminus S_{0.3}$. The Gaussian has the smallest errors
in both regions ($0.304$ and $0.191$). The corresponding values are
$0.340$ and $0.250$ for $\mathrm{ReLU}^{2}$, $0.358$ and $0.197$ for
$\tanh$, $0.409$ and $0.294$ for $\mathrm{ReLU}^{3}$, and $0.501$ and
$0.417$ for softplus.
...
Softplus and $\mathrm{ReLU}^{2}$ reach an upright neighbourhood from both
starts; the other fitted laws reach one only from $B$.}
\label{fig:rs_feedback}
\end{figure}

Two fitted laws reach the upright neighbourhood from both starts,
$\mathrm{ReLU}^{2}$ and softplus, and they do so at different costs
(Table~\ref{tab:rs_feedback_cost}). From $B$, $\mathrm{ReLU}^{2}$ brakes to
the upright at essentially the reference cost ($10.3$ vs.\ $10.2$); from
$A$ it reaches the upright with cost $76.3$ rather than the reference cost
$26.2$. Softplus reaches the upright at costs $69.8$ and $11.2$.
...
the $\mathrm{ReLU}^{2}$ law is closest to the reference, while all five fitted laws
...
\midrule
$\mathrm{ReLU}^{2} + \psi_{2}$ & $76.3$ & yes & $10.3$ & yes \\
$\mathrm{ReLU}^{3} + \psi_{3}$ & $1609.3$ & no & $10.2$ & yes \\
\bottomrule
\end{tabular}
...
In summary, the Gaussian gives the smallest validation and regional errors,
while $\mathrm{ReLU}^{2}$ gives the rollout closest to the reference. The
$\mathrm{ReLU}^{2}$ law reaches the upright from both starts, at
cost $10.3$ from $B$ and $76.3$ from $A$ against the reference $10.2$ and
...
decision from $A$ while they do not. Conversely, $\mathrm{ReLU}^{2}$ has
larger regional errors than the Gaussian and $\tanh$ away from the switching
set, yet gives the rollout closest to the reference. Thus the switching-set
error diagnoses the difficulty of approximating the discontinuous gradient,
but a reliable feedback assessment still requires a closed-loop test.
...
Theorem~\ref{thm:quantitative-insertion} gives squared one-step descent, while
Corollary~\ref{cor:certificate-rate} gives convergence of the excess above the
insertion threshold to zero and a best-iterate rate of order $n^{-1/2}$. The sufficient local
...
The Gaussian gives the smallest regional errors, while $\mathrm{ReLU}^{2}$
gives the closed-loop path closest to the reference. Softplus also reaches an
upright neighbourhood from both initial states, although at a larger cost from
$A$; the Gaussian, $\tanh$, and $\mathrm{ReLU}^{3}$ do so only from $B$.
The regional error and the closed-loop branch decision are therefore
complementary diagnostics rather than interchangeable rankings.
