...
In both algorithms the insertion step freezes the locations
...
\begin{proof}
We first establish attainment of the certificate maximum $B_p$.
...
\eqref{eq:explicit-certificate-radius}, using the sampled residual norm.
...
points at each iteration. Second, it overcomes the locality of the semismooth Newton method
by giving an initial outer weight that already guarantees a decrease.
...
can compromise feedback synthesis.
...
Under the $L^2$ loss the combined value--gradient error stays large for both powers
($\mathrm{Err}_{H^1} \approx 3.5 \times 10^{-1}$): the value-only fit
ignores $\nabla V$, so the atoms are placed without regard to the gradient
field. The $H^1$ loss lowers the combined error to
$\approx 2.4 \times 10^{-1}$ for both powers
