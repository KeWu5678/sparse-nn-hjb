...
less accurate than the best nonconvex curve at every support budget below $119$
atoms --- $1.70\times10^{-1}$ against $1.02\times10^{-1}$ at twenty atoms ---
and overtakes it at that width, eventually reaching
...
switching set, $\mathrm{ReLU}^{2}$ gives the smallest regional errors, while
softplus, $\mathrm{ReLU}^{2}$, and $\mathrm{ReLU}^{3}$ reach an upright
neighbourhood from both tested starts.
...
Three fitted laws reach the upright neighbourhood from both starts
(Table~\ref{tab:rs_feedback_cost}). Softplus does so at costs $69.8$ and
$11.2$ from $A$ and $B$, $\mathrm{ReLU}^{2}$ at $60.4$ and $10.1$, and
$\mathrm{ReLU}^{3}$ at $103.2$ and $10.3$. The Gaussian and $\tanh$ satisfy
the terminal criterion only from $B$.
...
\midrule
$\mathrm{ReLU}^{2} + \psi_{2}$ & $60.4$ & yes & $10.1$ & yes \\
$\mathrm{ReLU}^{3} + \psi_{3}$ & $103.2$ & yes & $10.3$ & yes \\
\bottomrule
\end{tabular}
...
Algorithm~\ref{alg:q} at the most accurate $\alpha$ for each $k$.}
...
We developed two sparse, gradient-augmented formulations for recovering value
functions from PMP data. For nonhomogeneous activations, simultaneous
normalization of the measure and dictionary controls growth on the unbounded
parameter domain without changing the represented function. Combined with a
concave penalty, this yields existence, finite support, an a priori width
bound, and a quantitative insertion certificate. For positively
$k$-homogeneous activations, radial scaling instead reduces the problem to the
unit sphere and induces a fractional-power penalty. The two formulations thus
lead to finite networks through different mechanisms and require different
insertion tests.

The numerical studies show that approximation error, sparsity, and feedback
performance need not give the same ranking. For the smooth Van der Pol value,
the Gaussian gives the smallest $H^1$ error among the tested nonhomogeneous
activations, while softplus uses fewer atoms; all tested nonhomogeneous
feedback laws stabilize the system at close to the reference cost. For the
pendulum, adding samples near the switching set produces no systematic or
material reduction in the regional error. $\mathrm{ReLU}^{2}$ gives the
smallest regional errors and, together with softplus and $\mathrm{ReLU}^{3}$,
reaches an upright neighbourhood from both tested initial states. The
Gaussian and $\tanh$ have smaller regional errors than softplus but fail from
one start. Regional $H^1$ error is therefore informative about gradient
approximation but does not by itself predict the closed-loop branch decision.

The analysis assumes exact global candidate maximization when deriving the
insertion rate, whereas the implementation uses a multistart local search.
Moreover, the experiments are two-dimensional and cover only a limited set of
control problems and activation families. Future work should address higher
dimensions, certified or more reliable candidate searches, and training and
validation criteria tied more directly to closed-loop behavior. For value
functions with switching sets, adaptive weighting or sampling on the two
sides of the discontinuity is another natural direction.
