...
attainable error. The traditional $\ell^1$ network is
...
that validation error alone does not determine feedback performance: the
Gaussian has lower validation error than softplus but fails from start $A$,
whereas softplus,
$\mathrm{ReLU}^{2}$, and $\mathrm{ReLU}^{3}$ reach an upright neighbourhood
from that start.
...
\end{subfigure}
\caption{Complementary performance summaries. Panel (a) shows the running
...
This is a terminal-time test, not an assertion of asymptotic stability.
...
$\alpha$ for each $k$. All fits use one atom per outer iteration and seed $42$.
The reference feedback compares $V_0$ and $V_{2\pi}$ using the nearest PMP
samples.}
