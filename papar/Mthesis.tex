...
\subsection{Optimality Condition and Finite Support of the Solution}
...
Assume $\phi_k$ is nondecreasing with $\phi_k(0) = 0$ (in particular, Assumption \ref{pen2} without the differentiability at zero suffices), assume (\ref{opt_dis_k}) admits at least one global minimizer, and let $\sigma$ be $k$-homogeneous with $k \ge 1$. Then (\ref{opt_dis_k}) is equivalent to
\begin{equation}
...
\begin{proof}
	...
	\end{align*}
	is feasible for the sphere-constrained problem and yields a value no larger than the optimum of (\ref{opt_dis_k}). Conversely, every sphere-feasible point is feasible for (\ref{opt_dis_k}), so the two optimal values coincide; in particular the projected point is a global minimizer of \eqref{discrete nonconvex problem}, and the rescaling maps minimizers of one problem to minimizers of the other, which is the meaning of equivalence here.
\end{proof}
...
\end{align*}
The first variation at $\bar{u}$ in a direction $u \in U$ is
\begin{align*}
	&\langle \hat{J}_u(\bar{u}), u \rangle_{U^*, U} = \langle y_u^*(\bar{u}) \cdot  J_y(y(\bar{u}), \bar{u}) + J_u(y(\bar{u}), \bar{u}), u \rangle_{U^*, U},
\end{align*}
where $*$ denotes the adjoint operator. Since $y(u)$ is implicitly defined in general and its derivative is expensive to form, we leverage the Lagrangian and implement the adjoint approach. Denote by $\mathcal{E}$ the codomain of $E$ and let $p \in  \mathcal{E}^*$ be the multiplier. Observe
\begin{align*}
...
\begin{align*}
    ...
    &= \langle 2 \beta u^* - g(y^*)^T p^*, u\rangle_{U^*, U} .
\end{align*}
...
Denote by $K$ the number of samples; the optimal neural network representation of the target is then supported on at most $K$ atoms.
