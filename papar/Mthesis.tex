...
$p_\varphi(\omega) := \langle \sigma(\cdot, \omega), \varphi\rangle_{L^2(D)}$
...
$|p_\varphi(\omega)| \le \kappa(\omega)\, \|\varphi\|_{L^2(D)}$ together with the
continuity of $\omega \mapsto \sigma(\cdot, \omega) \in L^2(D)$; the third from
$\langle \mathcal{N}\mu_n, \varphi\rangle_{L^2(D)} = \int_\Omega p_\varphi \, d\mu_n$,
...
$\mathcal{N}\bar{\mu} - V$ belongs to $H^2(D)$, the gradient channel
vanishes as well: writing $\nabla_x \sigma(x, \omega) = \rho'(a \cdot x + b)\, a = \nabla_x [\rho(a \cdot x + b)]$
and integrating by parts,
\begin{equation*}
	\langle \nabla(\mathcal{N}\bar{\mu} - V), \nabla_x \sigma(\cdot, \omega) \rangle_{L^2(D)}
	= -\langle \Delta (\mathcal{N}\bar{\mu} - V), \, \rho(a \cdot x + b) \rangle_{L^2(D)}
	+ \langle \partial_n (\mathcal{N}\bar{\mu} - V), \, \rho(a \cdot x + b) \rangle_{L^2(\partial D)},
\end{equation*}
and both terms tend to zero by the slab estimates of Lemma
\ref{lem:value_decay}, applied on $D$ and on the $(d-1)$-dimensional Lipschitz
boundary $\partial D$. In this case
$\limsup_{|\omega| \rightarrow \infty} |\bar{p}(\omega)| = 0 < \alpha$ and
\eqref{no_escape} holds. The residual lies in $H^2(D)$ for instance when
