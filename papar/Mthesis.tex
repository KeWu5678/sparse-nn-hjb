...
\begin{proof}
	...
	\begin{equation*}
		\nu_h \overset{*}{\rightharpoonup} \nu, \qquad
		\lambda_h \overset{*}{\rightharpoonup} \lambda, \qquad
		\nu + \lambda = \mu .
	\end{equation*}
	We estimate the two parts separately. \\
	\textbf{Heavy part:} \par
	Each $\nu_h$ is purely atomic with at most $k$ atoms. By lemma 3.6 in \cite{bouchitte1990new}, the set of measures with at most $k$ atoms is sequentially weak-* closed --- hence $\nu$ is purely atomic with at most $k$ atoms --- and on this set the functional $\lambda \mapsto \sum_{\omega} \phi(|\lambda|(\{\omega\}))$ is sequentially weak-* l.s.c., since $\phi$ is continuous and subadditive (Lemma \ref{firstorderestimate}). Therefore
	\begin{align*}
		\liminf_{h} \Phi_1(\nu_h)
		= \liminf_h  \sum_{ \omega \in \atom \nu_h } \phi(|\nu_{h}|(\{\omega\}))
		\ge \sum_{ \omega \in \atom \nu } \phi(|\nu|(\{\omega\})) = \Phi_1 ( \nu).
	\end{align*}
	...
	\end{equation*}
	Every atom of $\lambda_h$ has mass below $\epsilon$, and the continuous part enters $\Phi_1$ with weight one, so
	\begin{equation*}
	\begin{aligned}
		\Phi_1(\lambda_h)
		&= \|\lambda_{h, \cont}\|_{\mathcal{M}(\Omega)} + \sum_{\omega \in \atom \lambda_h} \phi(|\lambda_{h}|(\{\omega\}))\\
		&\ge (1 - \delta) \bigg( \|\lambda_{h, \cont}\|_{\mathcal{M}(\Omega)} + \sum_{\omega \in \atom \lambda_h} |\lambda_{h}|(\{\omega\}) \bigg)
		= (1 - \delta)\, \|\lambda_h\|_{\mathcal{M}(\Omega)}.
	\end{aligned}
	\end{equation*}
	By the weak-* l.s.c. of the norm $\|\cdot\|_{\mathcal{M}(\Omega)}$ and by $\Phi_1 \le \|\cdot\|_{\mathcal{M}(\Omega)}$ (Lemma \ref{envolop}),
	\begin{equation*}
		\liminf_h \Phi_1(\lambda_h) \ge (1 - \delta)\, \|\lambda\|_{\mathcal{M}(\Omega)} \ge (1 - \delta)\, \Phi_1(\lambda).
	\end{equation*}

	\quad\\
	\textbf{Recombination:} \par
	The functional $\Phi_1$ is subadditive: for $\mu = \nu + \lambda$ with $\nu$ purely atomic we have $\mu_{\cont} = \lambda_{\cont}$, every atom of $\mu$ is an atom of $\nu$ or of $\lambda$, and $\phi(|\nu + \lambda|(\{\omega\})) \le \phi(|\nu|(\{\omega\})) + \phi(|\lambda|(\{\omega\}))$ by the monotonicity and subadditivity of $\phi$; hence $\Phi_1(\mu) \le \Phi_1(\nu) + \Phi_1(\lambda)$. Combining the two parts,
	\begin{align*}
		\ell = \lim_h \Phi_1(\mu_h)
		\ge \liminf_h \Phi_1(\nu_h) + \liminf_h \Phi_1(\lambda_h)
		\ge \Phi_1(\nu) + (1 - \delta)\, \Phi_1(\lambda)
		\ge (1 - \delta)\, \Phi_1(\mu).
	\end{align*}
...
$w_0(\omega) := \|\rho(a \cdot x + b)\|_{L^2(D)}$. Then $w_0 \in C_0(\Omega)$.
Consequently, for every $\varphi \in L^2(D)$ the function
$p^0_\varphi(\omega) := \langle \rho(a \cdot x + b), \varphi\rangle_{L^2(D)}$
...
\end{lemma}
\begin{proof}
Boundedness is clear from $w_0 \le \|\rho\|_\infty$. Let $R_D > 0$ be such that
$D \subset [-R_D, R_D]^d$. For $|a| \ge 1$ choose coordinates with
$a/|a| = e_1$; then
\begin{equation*}
...
\end{equation*}
For $|a| \le R$ and $|b| \to \infty$ we have
$\inf_{x \in D} |a \cdot x + b| \ge |b| - R\,R_D\sqrt{d} \to \infty$ and
$\rho(z) \to 0$ as $|z| \to \infty$; hence $w_0(\omega) \to 0$ as
$|\omega| \to \infty$ in either regime. Continuity of $w_0$ follows from
dominated convergence. The second claim follows from
$|p^0_\varphi(\omega)| \le w_0(\omega)\, \|\varphi\|_{L^2(D)}$ together with the
continuity of $\omega \mapsto \rho(a \cdot x + b) \in L^2(D)$; the third from
$\langle \mathcal{N}\mu_n, \varphi\rangle_{L^2(D)} = \int_\Omega p^0_\varphi \, d\mu_n$,
...
(i) For $\rho = \tanh$ take $V \equiv 1$ and $\mu_n = \delta_{(a, b_n)}$ with
...
contribution.
(ii) For $\rho(z) = \log(1 + e^z)$ (softplus) the measures
...
gradient channels,
\begin{equation*}
	\bar{p}(\omega) = \langle \mathcal{N}\bar{\mu} - V, \, \rho(a \cdot x + b) \rangle_{L^2(D)}
	+ \langle \nabla(\mathcal{N}\bar{\mu}) - \nabla V, \, \rho'(a \cdot x + b)\, a \rangle_{L^2(D)}.
\end{equation*}
The value channel always vanishes at infinity:
$|\langle \mathcal{N}\bar{\mu} - V, \rho(a \cdot x + b)\rangle| \le \|\mathcal{N}\bar{\mu} - V\|_{L^2(D)} \, w_0(\omega) \rightarrow 0$
by Lemma \ref{lem:value_decay}. If in addition the residual
$w := \mathcal{N}\bar{\mu} - V$ belongs to $H^2(D)$, the gradient channel
vanishes as well: writing $\rho'(a \cdot x + b)\, a = \nabla_x [\rho(a \cdot x + b)]$
and integrating by parts,
\begin{equation*}
	\langle \nabla w, \nabla_x[\rho(a \cdot x + b)] \rangle_{L^2(D)}
	= -\langle \Delta w, \rho(a \cdot x + b) \rangle_{L^2(D)}
	+ \langle \partial_n w, \rho(a \cdot x + b) \rangle_{L^2(\partial D)},
\end{equation*}
