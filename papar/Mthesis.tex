...
In this example we consider the pendulum swing-up problem from \cite{han2024nonsmooth}.
We write
\begin{equation*}
...
The generator therefore detects the switching curve by intersecting equal-value contours with their $2\pi$-shifted copies and then restricts the PMP trajectories to a consistent smooth branch before forming the final $(x,V,dV)$ dataset.
This is the feature that makes the pendulum benchmark a stress test for the gradient-augmented sparse neural approximation: the value can be continuous while its gradient changes sharply across the switching set.
\begin{figure}[H]
\centering
\captionsetup[subfigure]{justification=centering}
